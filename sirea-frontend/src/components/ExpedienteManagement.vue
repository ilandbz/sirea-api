  ...
  <div class="container-fluid">
    <div class="card shadow-sm border-0 rounded-4">
      <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        ...
        <button class="btn btn-primary rounded-pill px-4" @click="abrirModalNuevoExpediente">
              ...
              <tr v-for="exp in expedientes" :key="exp.id">
                    ...
                    >
                      <div class="d-flex align-items-center justify-content-between">
                        <div>
                          ...
                          <button
                            class="btn btn-sm text-danger border-0 p-0 ms-1"
                            title="Quitar Parte"
                            @click="quitarParte(exp.id, usuario.id)"
                          >
                ...
                <td class="text-end pe-4">
                  ...
                  <button
                    ...
                    @click="eliminarExpediente(exp.id)"
                  >
...
// Genera código único con tipo + fecha + hora + milisegundos + aleatorio
const generarCodigoExpediente = (tipo = 'IA') => {
  const now = new Date();
  ...
  return `${tipo}-${yyyy}${mm}${dd}-${hh}${mi}${ss}${ms}-${random}`;
};
...
const resetForm = () => {
  ...
  form.demandante = '';
  form.demandado = '';
  form.codigo = generarCodigoExpediente(form.tipo);
};

const abrirModalNuevoExpediente = () => {
  resetForm();
  mostrarModal.value = true;
};

// Si el usuario cambia el tipo, regeneramos el código con ese prefijo
watch(() => form.tipo, (nuevoTipo) => {
  form.codigo = generarCodigoExpediente(nuevoTipo);
});

// --- LÓGICA DE CREACIÓN DE EXPEDIENTE ---
const guardarExpediente = async () => {
  try {
    // Reforzamos antes de guardar por si el modal quedó abierto mucho rato
    form.codigo = generarCodigoExpediente(form.tipo);
...
const getBadgeTipo = (tipo) => {
  const types = {
    IA: 'bg-primary',
    JRD: 'bg-warning text-dark',
    CONCILIACION: 'bg-info text-white'
  };
